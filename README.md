...
At a high level, `dsldensify` supports **two complementary approaches**
to conditional density estimation under a common cross-validation
interface.
...
Direct models estimate $f(A \mid W)$ without discretization, for
example:

-   homoskedastic Gaussian regression,
-   flexible GAMLSS models with covariate-dependent parameters.

These models are evaluated using the same cross-validated log-density
loss as hazard-based learners and can compete directly with them in
model selection.
